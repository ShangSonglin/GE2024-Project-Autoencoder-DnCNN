...
if __name__ == '__main__':
    ...
    autoencoder.load_state_dict(torch.load('autoencoder_weights.pth', map_location=device, weights_only=False))
    autoencoder.eval()

    # 加载DnCNN模型
    dncnn = DnCNN().to(device)
    dncnn.load_state_dict(torch.load(os.path.join(args.model_dir, args.model_name), map_location=device, weights_only=False))
    dncnn.eval()
        ...
        for im in os.listdir(set_dir):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                ...
                if args.save_result:
                    ...
                    save_result(x_dncnn, path=result_path)  # save the denoised image
