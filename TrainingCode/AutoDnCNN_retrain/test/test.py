...
if __name__ == '__main__':
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        # 保存原始图片
        save_result(data.squeeze().detach().numpy(), os.path.join(args.result_dir, f'original_{batch_idx}.png'))
        log(f'Saved original image: original_{batch_idx}.png')

        # 添加高斯噪声
        noisy_images = add_gaussian_noise(data)
        save_result(noisy_images.squeeze().detach().numpy(), os.path.join(args.result_dir, f'noisy_{batch_idx}.png'))
