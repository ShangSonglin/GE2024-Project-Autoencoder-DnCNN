...
import DnCNN_datagenerator as dg
from DnCNN_datagenerator import DenoisingDataset
...
if __name__ == '__main__':
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        if xs.ndim == 3:  # 如果数据是三维的（高度、宽度、通道）
            xs = np.expand_dims(xs, axis=3)  # 扩展为四维
        elif xs.ndim != 4:
            raise ValueError("数据维度不正确，期望三维或四维数组")
